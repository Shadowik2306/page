...
export default {
  ...
  methods: {
    edit_item() {
      let vm = this;
      fetch("http://localhost:8081/items/" + vm.id, {
        method: 'PUT',
        headers: {
      ...
      fetch("http://localhost:8081/storage_item")
          ...
          .then(function (data) {
            ...
            let sum = 0;
            data.forEach(props => {
              if (props.id_item == vm.id) {
                fetch("http://localhost:8081/storage_item/" + props.id, {
                  method: 'PUT',
                  headers: {
                    'Content-Type': 'application/json'
                  },
                  body: JSON.stringify({'id': props.id, 'id_storage': props.id_storage,
                    'id_item': props.id_item, 'count': vm.count})
                })
                    .then((response) => {
                      response.json()
                    })
